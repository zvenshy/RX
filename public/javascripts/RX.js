...
RX.fn = RX.prototype = (function () {
    //类数组对象数组化
    var toArray = function (obj, start, end) {
...
//扩展对象
RX.fn.extend = function (obj, source, over) {
    var toArray = RX.fn.toArray
    ,   args    = toArray(arguments)
    ,   over    = over || false  //是否覆盖
    ,   key;
    for ( key in obj ) {
        if (over || !( key in source ) ) {
            source[ key ] = obj[ key ];
        }
    }
    return source;
...
RX.modules.animate = function (rx) {
        ...
        var ani = setInterval (function () {
            ...
            if (pre >= 1) {
                for ( ; key = changeSty[ i++ ]; ) {
                    ...
                    scroll.style[ key ] = end[ key ] + 'px';
                    clearInterval(ani);
                    return;
                }
            }
            for ( ; key = changeSty[ i++ ]; ) {
                scroll.style[ key ] = begin[ key ] +
                    easings[easing](pre)*change[ key ] + 'px';
            }
        }, everytime);  
    };
    
    var show = function (obj, type) {
        obj.style.display = type || 'block';
        return obj;
    };
    ...
    var togl = function ( obj, type) {
        var dis = compStyle( obj, 'display');
        if (dis === 'none') {
            show( obj, type );
        } else {
            hide ( obj, type );
...
RX.modules.ajax = function (rx) {
    //opts中参数(json格式)
    //url:...
    //data:post data json格式
    //method:GET/POST...
    //contentType:请求头
    //charset:请求头编码
    //type:响应数据类型
        ...
        xhr.onreadystatechange = function () {
            if ( xhr.readyState === 4 && ( xhr.status >= 200 && xhr.status <= 300 ) || 
                ...
                var success = opts['success'] || function () {
                    console.log('Don\'t have success callback!');
                }
                ,   type      = opts['type'] || xhr.getResponseHeader('Content-Type') || 'text/plain'
                ,   typeSplit = type.split(';')[0].split('/')[1];
                success( parse[ typeSplit ]() );
            }
        }
...
RX.modules.browser = function (rx) {
    //From https://github.com/RubyLouvre/mass-Framework/blob/1.4/more/brower.js & jQuery
